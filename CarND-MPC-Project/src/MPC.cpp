...
AD<double> polyeval_slope_cppad(Eigen::VectorXd coeffs, AD<double> x) {
  AD<double> result = 0.0;
...
std::tuple<vector<double>, vector<double>> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  bool ok = true;
  size_t i;
  ...
  // two vectors to output predicted positions and current control values
  vector<double> pos, ctrl;

  for(size_t i = 0; i < N - 1; i ++) {
    pos.push_back(solution.x[x_start + 1 + i]);
    pos.push_back(solution.x[y_start + 1 + i]);
  }

  ctrl.push_back(solution.x[delta_start]);
  ctrl.push_back(solution.x[a_start]);

  return std::make_tuple(pos, ctrl);
}
